...
architecture rtl of power_integration is
    ...
    constant power_sum_bits:	integer := 18; --actually 25 but this fits into the io regs
	 ...
	 function get_div_val(factor : integer) return integer is
		begin
		 if factor = 1 then
			return 4;
		 else
    ...
    signal power_sum_mid_0 : power_array:=(others=>(others=>'0'));
    signal power_sum_mid_1 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_2 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_3 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_4 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_5 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_6 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_7 : power_array:=(others=>(others=>'0'));
	 signal power_sum_mid_8 : power_array:=(others=>(others=>'0'));
    ...
    signal power_sum_a : bigger_power_array:=(others=>(others=>'0'));
    signal power_sum_b : bigger_power_array:=(others=>(others=>'0'));
	 signal power_sum_c : bigger_power_array:=(others=>(others=>'0'));
	 signal power_sum_d : bigger_power_array:=(others=>(others=>'0'));
    ...
    --sample_o<=std_logic_vector(phased_beam_waves(0,0));
        ...
        for i in 0 to NUM_BEAMS-1 loop --loop over beams
            for j in 0 to NUM_SAMPLES*INTERP_FACTOR-1 loop --for j in 16 to phased_sum_length-1 loop
                phased_power(i,j)<=phased_beam_waves(i,j)*phased_beam_waves(i,j); --for mult
...
begin		
    ...
    if rst_i = '1' then
        ...
        --power_sum_16 <= (others=>(others=>'0'));
			...
			power_sum_mid_0 <= (others=>(others=>'0'));
			power_sum_mid_1 <= (others=>(others=>'0'));
			power_sum_mid_2 <= (others=>(others=>'0'));
			power_sum_mid_3 <= (others=>(others=>'0'));
			power_sum_mid_4 <= (others=>(others=>'0'));
			power_sum_mid_5 <= (others=>(others=>'0'));
			power_sum_mid_6 <= (others=>(others=>'0'));
			power_sum_mid_7 <= (others=>(others=>'0'));
			
			power_sum_a <= (others=>(others=>'0'));
			power_sum_b <= (others=>(others=>'0'));
			power_sum_c <= (others=>(others=>'0'));
			power_sum_d <= (others=>(others=>'0'));
			  ...
			  for i in 0 to NUM_BEAMS-1 loop
					...
					if NUM_SAMPLES*INTERP_FACTOR = 16 and NUM_POWERS = 4 then
						...
						power_sum_1(i)<=resize(phased_power(i,4),num_power_bits)+resize(phased_power(i,5),num_power_bits)+resize(phased_power(i,6),num_power_bits)+resize(phased_power(i,7),num_power_bits);
						 ...
						 power_sum_mid_0(i) <= power_sum_0(i) + power_sum_1(i) + power_sum_2(i) + power_sum_3(i);
						 power_sum_mid_1(i) <= power_sum_1(i) + power_sum_2(i) + power_sum_3(i) + power_sum_4(i);
						 power_sum_mid_2(i) <= power_sum_2(i) + power_sum_3(i) + power_sum_4(i) + power_sum_5(i);
						 power_sum_mid_3(i) <= power_sum_3(i) + power_sum_4(i) + power_sum_5(i) + power_sum_6(i);
											 
						 power_sum_mid_4(i) <= power_sum_mid_0(i);
						 power_sum_mid_5(i) <= power_sum_mid_1(i);
						 power_sum_mid_6(i) <= power_sum_mid_2(i);
						 power_sum_mid_7(i) <= power_sum_mid_3(i);
											 
						 power_sum_a(i) <= resize(power_sum_mid_0(i),power_sum_a(i)'length)+power_sum_mid_4(i);
						 power_sum_b(i) <= resize(power_sum_mid_1(i),power_sum_b(i)'length)+power_sum_mid_5(i);
						 power_sum_c(i) <= resize(power_sum_mid_2(i),power_sum_c(i)'length)+power_sum_mid_6(i);
						 power_sum_d(i) <= resize(power_sum_mid_3(i),power_sum_d(i)'length)+power_sum_mid_7(i);
						...
						--							- power_sum_11(i) - power_sum_12(i) - power_sum_13(i) - power_sum_14(i);
					...
					elsif NUM_SAMPLES*INTERP_FACTOR=8 and NUM_POWERS=2 then -- erg, default 4 samples and 2 powers for now. add in other options if different ones neededNUM_SAMPLES*INTERP_FACTOR = 4 and NUM_POWERS = 2
						 power_sum_0(i)<=resize(phased_power(i,0),num_power_bits)+resize(phased_power(i,1),num_power_bits)+resize(phased_power(i,2),num_power_bits)+resize(phased_power(i,3),num_power_bits);
						 power_sum_1(i)<=resize(phased_power(i,4),num_power_bits)+resize(phased_power(i,5),num_power_bits)+resize(phased_power(i,6),num_power_bits)+resize(phased_power(i,7),num_power_bits);
						 ...
						 power_sum_mid_2(i) <= power_sum_mid_0(i);				 
						 power_sum_mid_3(i) <= power_sum_mid_1(i);
						 power_sum_mid_4(i) <= power_sum_mid_2(i);
						 power_sum_mid_5(i) <= power_sum_mid_3(i);
						 power_sum_mid_6(i) <= power_sum_mid_4(i);
											 
						 power_sum_a(i) <= resize(power_sum_mid_0(i),power_sum_a(i)'length)+power_sum_mid_4(i);
						 power_sum_b(i) <= resize(power_sum_mid_1(i),power_sum_a(i)'length)+power_sum_mid_5(i);
					...
					elsif NUM_SAMPLES*INTERP_FACTOR=8 and NUM_POWERS=4 then
						 power_sum_0(i)<= resize(phased_power(i,0),num_power_bits)+resize(phased_power(i,1),num_power_bits);
						 power_sum_1(i)<= resize(phased_power(i,2),num_power_bits)+resize(phased_power(i,3),num_power_bits);
						 ...
						 power_sum_mid_6(i) <= power_sum_mid_2(i);
						 power_sum_mid_7(i) <= power_sum_mid_3(i);
						 power_sum_mid_8(i) <= power_sum_mid_4(i);
											 
						 power_sum_a(i) <= resize(power_sum_mid_0(i),power_sum_a(i)'length)+power_sum_mid_4(i);
						 power_sum_b(i) <= resize(power_sum_mid_1(i),power_sum_a(i)'length)+power_sum_mid_5(i);
						 power_sum_c(i) <= resize(power_sum_mid_2(i),power_sum_a(i)'length)+power_sum_mid_6(i);
						 power_sum_d(i) <= resize(power_sum_mid_3(i),power_sum_a(i)'length)+power_sum_mid_7(i);

					
					end if;

					-- hard code. running_sum_a or power_sum_a

					if power_sum_a(i)(num_div-1 downto 0) >= round_val then --x"10"
						 avg_power(i,0)<=resize(unsigned(power_sum_a(i)(power_sum_a(0)'length-1 downto num_div)),avg_power(0,0)'length)+1;
					else
						 avg_power(i,0)<=resize(unsigned(power_sum_a(i)(power_sum_a(0)'length-1 downto num_div)),avg_power(0,0)'length);
					end if;

					if power_sum_b(i)(num_div-1 downto 0) >= round_val then
						 avg_power(i,1)<=resize(unsigned(power_sum_b(i)(power_sum_b(0)'length-1 downto num_div)),avg_power(0,0)'length)+1;
					else
						 avg_power(i,1)<=resize(unsigned(power_sum_b(i)(power_sum_b(0)'length-1 downto num_div)),avg_power(0,0)'length);
					end if;


					if NUM_POWERS = 4 then

						 if power_sum_c(i)(num_div-1 downto 0) >= round_val then
							  avg_power(i,2)<=resize(unsigned(power_sum_c(i)(power_sum_c(0)'length-1 downto num_div)),avg_power(0,0)'length)+1;
						 else
							  avg_power(i,2)<=resize(unsigned(power_sum_c(i)(power_sum_c(0)'length-1 downto num_div)),avg_power(0,0)'length);
						 end if;

						 if power_sum_d(i)(num_div-1 downto 0) >= round_val then
							  avg_power(i,3)<=resize(unsigned(power_sum_d(i)(power_sum_d(0)'length-1 downto num_div)),avg_power(0,0)'length)+1;
						 else
							  avg_power(i,3)<=resize(unsigned(power_sum_d(i)(power_sum_d(0)'length-1 downto num_div)),avg_power(0,0)'length);
						 end if;
					end if;
        end loop;
		  end if;
